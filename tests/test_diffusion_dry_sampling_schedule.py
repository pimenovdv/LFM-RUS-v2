...
def test_dry_sampling_schedules(mocker):
    ...
    def side_effect(*args, **kwargs):
        ...
        class MockOut:
            def __init__(self):
                self.last_hidden_state = torch.rand((1, 5, 12))
        return MockOut()
    ...
    outputs = model.generate(
        ...
        steps=3,
        dry_multiplier=0.5,
        dry_multiplier_schedule="linear",
        min_dry_multiplier=0.1
    )
    assert outputs is not None
    ...
    outputs = model.generate(
        ...
        steps=3,
        dry_multiplier=0.5,
        dry_multiplier_schedule="cosine"
    )
    assert outputs is not None
    ...
    outputs = model.generate(
        ...
        steps=3,
        dry_multiplier=0.5,
        dry_multiplier_schedule="exponential"
    )
    assert outputs is not None
    ...
    outputs = model.generate(
        ...
        steps=3,
        dry_multiplier=0.5,
        dry_multiplier_schedule="cyclic"
    )
    assert outputs is not None
