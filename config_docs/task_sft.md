...
* **`lora`** (словарь): Конфигурация для LoRA (если `use_lora: true`).
  ...
  * `task_type` (строка): Тип задачи, для `peft`. Возможные варианты:
    * `"CAUSAL_LM"`: Языковое моделирование с причинно-следственной связью (Causal Language Modeling). Этот вариант используется для большинства современных генеративных языковых моделей (LLM).
  * *Пример*:
    ```yaml
    r: 8
    lora_alpha: 32
